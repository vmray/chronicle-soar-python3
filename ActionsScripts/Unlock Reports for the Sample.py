...
from Conf import VMRayConfig, INTEGRATION_NAME, UNLOCK_REPORT_SCRIPT_NAME
...
def main():
    ...
    siemplify.script_name = UNLOCK_REPORT_SCRIPT_NAME
    
    siemplify.LOGGER.info("----------------- Main - Param Init -----------------")
    
    # Initializing integration parameters for Unlock Report for the Sample Action
    ...
    # initializing action specific parameters for Unlock Report for the Sample Action
    ...
    try:
        ...
        unlock_status = vmray.unlock_reports(sample_id)
        ...
        if unlock_status:
            ...
            output_message = "Reports successfully unlocked for sample %s" % sample_id
        
            # Set a simple result value, used for playbook if\else and placeholders.
            result_value = True
    
            siemplify.LOGGER.info("%s action finished successfully." % UNLOCK_REPORT_SCRIPT_NAME)
        else:
            # used to flag back to siemplify system, the action final status
            status = EXECUTION_STATE_FAILED  
        
            # human readable message, showed in UI as the action result
            output_message = "Couldn't unlock reports for sample %s." % sample_id
        
            # Set a simple result value, used for playbook if\else and placeholders.
            result_value = False
            
            siemplify.LOGGER.info("%s action failed." % UNLOCK_REPORT_SCRIPT_NAME)
    except Exception as err:
        # used to flag back to siemplify system, the action final status
        status = EXECUTION_STATE_FAILED
        
        # human readable message, showed in UI as the action result
        output_message = "Couldn't unlock reports for sample %s. Error: %s" % (sample_id, err)
        
        # Set a simple result value, used for playbook if\else and placeholders.
        result_value = False
        
        siemplify.LOGGER.error("%s action finished with error." % UNLOCK_REPORT_SCRIPT_NAME)
